...
export default function RSVP() {
  ...
  return (
    <section id="rsvp" className="flex flex-col items-center gap-8 bg-sage-50 px-6 py-20">
      ...
      {status === 'done' ? (
        <Reveal className="rounded-md border border-sage-200 bg-white px-6 py-8 text-center">
          <p className="font-serif text-base text-sage-700">소중한 답변 감사합니다 🌿</p>
        </Reveal>
      ) : (
        <Reveal delay={0.1} className="w-full">
          <form onSubmit={handleSubmit} className="flex w-full flex-col gap-4">
            ...
            <input
              ...
              required
              placeholder="이름"
              className="rounded-md border border-sage-200 bg-white px-4 py-2.5 text-sm outline-none focus:border-sage-500"
            />
            <input
              name="guests"
              type="number"
            ...
            <textarea
              ...
              rows={3}
              className="resize-none rounded-md border border-sage-200 bg-white px-4 py-2.5 text-sm outline-none focus:border-sage-500"
            />
