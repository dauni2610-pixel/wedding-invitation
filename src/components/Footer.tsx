  ...
  return (
    <footer className="flex flex-col items-center gap-6 bg-sage-800 px-8 py-20 text-center text-cream">
      <Reveal className="flex flex-col items-center gap-3">
        <p className="font-script text-2xl">{closingMessage.title}</p>
      ...
      <Reveal delay={0.15}>
        <button
          type="button"
          onClick={handleShare}
          className="rounded-full border border-cream/40 px-6 py-2.5 text-xs tracking-widest text-cream/90 transition-colors hover:bg-cream/10"
        >
          {copied ? '링크가 복사되었습니다' : '청첩장 공유하기'}
        </button>
      </Reveal>

      <p className="mt-6 text-[10px] tracking-widest text-cream/40">
