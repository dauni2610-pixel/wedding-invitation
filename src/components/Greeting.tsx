  ...
  return (
    <section className="flex flex-col items-center gap-8 px-8 py-20">
      ...
      <Reveal delay={0.2} className="flex flex-col items-center gap-1 pt-2 text-sm text-ink/70">
        <p>
          {groom.fatherName} · {groom.motherName}의 아들 <span className="font-medium text-ink">{groom.name}</span>
        </p>
        <p>
          {bride.fatherName} · {bride.motherName}의 딸 <span className="font-medium text-ink">{bride.name}</span>
        </p>
      </Reveal>
    </section>
  )
